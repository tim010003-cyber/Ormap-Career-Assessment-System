...
| 共用模組 | 尚未存在 | ✅ `firebase-init.js` / `auth-utils.js` / `gas-mailer.js` / `config.js` 皆存在且被引用 |
| Phase 9.1 諮詢師管理安全機制 | 完全未提及 | ✅ 已實作（降級兩步驟 + 自我保護 + 最後一個 SA 保護），見第七章 |
| GAS 信件 type 名稱 | `assessment_done` / `report_ready` | 呼叫端確實用這兩個；但 `gas-mailer.js` 註解寫成 `assessment_completed` / `report_delivery`（不一致，見第十四章） |
...
| 發信 | Google Apps Script（GAS）Web App，透過 `new Image()` GET 觸發 |
...
### `gas-mailer.js`
- `export GAS_MAILER_URL` + `callGasMailer(type, data)`
- 用 `new Image().src` 觸發 GET（天然跟隨 GAS 302 redirect、繞開 CORS；`fetch`+`no-cors` 會靜默失敗——踩過的坑）
- 永遠 `resolve(true)`（fire-and-forget），3 秒 timeout
- URL 未設定（含 `__`）時回 `false` 並 warn
- ⚠️ 檔頭註解寫的四種 type（`invite_counselor` / `invite_student` / `assessment_completed` / `report_delivery`）**與實際呼叫端不一致**，詳見第十四章
...
### 諮詢師邀請
```
Super Admin counselors.html → 新增（email + name）→ 寫 pending 文件（email 命名）→ GAS 寄邀請信（連結指向 /admin.html）
...
### 完成寫入
`saveToFirestore()` → `addDoc(assessments, {...rawData...})`，`counselorAccess = !!window._wbSessionId`；清 localStorage + 刪 `user_progress` + 刪 `whiteboard_active`；`callGasMailer('assessment_done', {...})`。
...
### 完成寫入
`addDoc(assessments, { toolType:'HTI', clientInfo{name,email}, scores, topTraits, summary, uid, counselorAccess, createdAt })`；清草稿 + 刪 `whiteboard_active`；`callGasMailer('assessment_done', ...)` → 導回 dashboard。
...
- 部署於 PO 個人 Google 帳號的 GAS Web App
- 觸發：`new Image().src = GAS_MAILER_URL + '?' + URLSearchParams({ type, payload })`（見 `gas-mailer.js`）

### 四種信件 — ⚠️ type 名稱現況與不一致

| 觸發場景 | 呼叫端實際傳的 type | 位置 | gas-mailer.js 註解寫的 |
|---|---|---|---|
| 邀請諮詢師 | `invite_counselor` | counselors.html | `invite_counselor` ✅一致 |
| 邀請學員入白板 | `invite_student` | whiteboard.html:774 | `invite_student` ✅一致 |
| 學員完成評測 | `assessment_done` | twa.html:2735 / hti.html:1337 | `assessment_completed` ❌**不一致** |
| 諮詢結束報告就緒 | `report_ready` | whiteboard.html:596 | `report_delivery` ❌**不一致** |

**現況判讀**：所有呼叫端彼此一致（`assessment_done` / `report_ready`），也與 PRD v3.7 記載的 GAS 後端 switch 一致。因此**最可能是 `gas-mailer.js` 檔頭註解與 CLAUDE.md 寫錯**，而非信寄不出。但**必須對照實際部署的 `gas_mailer.gs` 確認** GAS 後端 switch 到底認哪組字串。風險：若有人照註解「訂正」呼叫端 → 信默默寄不出。

### 已知待辦
- `assessment_done` 目前固定通知 Super Admin，尚未依 `assignedCounselorUid` 路由給對應諮詢師（CLAUDE.md 亦記為未完成 debt）
- 商業化後改用自訂網域發信（v4+）
...
- Pendulum 擺盪決策工具、付費解鎖、自訂發信網域（v4+）
...
### 🔴 高
- **A1 admin.html 儲存型 XSS**：`assessments` 為 `create` 全開，匿名者可寫入含 `<img onerror>` 的 `clientInfo.name` 並指定 `assignedCounselorUid` 鎖定目標諮詢師。admin.html 以 template literal + `innerHTML` 渲染且**未轉義**（`renderClientList`/`renderSummary`/`renderAssessments`）。諮詢師（尤其 Super Admin 看全部）開後台時 payload 在其登入 session 執行。對照組 report.html 全程走 `escHtml()`。**建議優先修**：admin.html 所有使用者資料渲染前加轉義。
- **A2 assessments 讀取規則過寬（隱私 / 授權未落實）**：`allow read: if request.auth != null` 讓任何登入者（含被撤銷授權的諮詢師）可直接讀任何評測。`counselorAccess` 撤銷只在前端隱藏頁面，不保護資料。**建議**：Rules 加 `(isOwner || isSuperAdmin || (isCounselor && assignedCounselorUid==uid && counselorAccess != false))`。

### 🟠 中
- **A3 GAS type 名稱不一致**：呼叫端 `assessment_done`/`report_ready` vs `gas-mailer.js` 註解 `assessment_completed`/`report_delivery`。需對照實際 GAS 後端統一，並修正 `gas-mailer.js` 註解 + CLAUDE.md。
- **A4 whiteboard 結束後兩個競爭 redirect**：`confirmEnd` 排 `admin.html`（1800ms），session snapshot listener 又排 `dashboard.html`（2000ms），落點不確定。擇一。
- **A5 whiteboard「儲存備忘」是空操作**：只 toast「備忘已儲存」，textarea 內容從未寫入 `counselor_notes`，reload 即遺失（誤導性成功 + 靜默資料遺失）。
- **A6 HTI「能量聚焦度」(module 2A) 算了卻沒渲染**：`hti.html:905-912` 算出 `focus*` 變數，但報告 innerHTML 只放了 2B 內在協作度，整段 2A 從報告中遺失。
...
技術：純 HTML/JS + Tailwind(CDN) + Chart.js + Lucide@1.8.0 + Firebase 10.8.1（Auth+Firestore）+ GAS 發信
共用模組（必 import，勿重造）：firebase-init.js / auth-utils.js / gas-mailer.js / config.js（tools/ 用 ../）
...
GAS：new Image() 觸發 GET，type = invite_counselor/invite_student/assessment_done/report_ready（⚠️對照 gas_mailer.gs 確認）
...
⚠️ 動手前先看本規格書第十五章「已知問題」，尤其 A1（admin XSS）與 A2（assessments 讀取規則）
```
