...
✅ `report.html` 已於 2026-07-16 改用本模組（A12），全站權限判定 5/5 統一。另有 `pendingKeyForEmail(email)` export（A14 新增，counselors 建 pending 文件與遷移查找共用）。
...
- ✅ 權限判定已改用 auth-utils（2026-07-16，A12）
...
- ~~report.html 權限判定改用 auth-utils（D4）~~ ✅ 2026-07-16 完成
...
### 🟢 低 / 整潔
- ~~**A7 counselors.html onclick 注入面**~~：✅ 2026-07-16 修復——新增 `esc()`；全部 onclick 改 `data-id` 屬性傳值（不做 JS 字串插值），名稱由 `counselorNameById` map 查回；name/email/首字母渲染前轉義。
- ~~**A8 whiteboard 重複 `#student-screen` DOM**~~：✅ 2026-07-16 修復——`showStudentWelcome` 建立前先 `remove()` 既有節點。
- ~~**A9 whiteboard 結束綁定非原子**~~：✅ 2026-07-16 修復——改 `writeBatch` 原子提交（全部成功或全部不寫）。
- ~~**A10 dashboard 監聽器 / userName**~~：✅ 2026-07-16 修復——`onSnapshot` 保留 unsubscribe + `beforeunload` 解除；`userName` 退回 email。
- ~~**A11 HTI dead code / div**~~：✅ 2026-07-16 修復——`skipToReport()` 已移除；div 不對稱已在 A6 一併修正。
- ~~**A12 report.html inline 權限語意分歧（D4）**~~：✅ 2026-07-16 修復——改 import `isCounselor` / `isSuperAdmin`（auth-utils），全站權限判定 5/5 統一。
- ~~**A13 admin.html dead code**~~：✅ 2026-07-16 修復——`ncCopyInvite`、`openWhiteboard` 已移除。
- ~~**A14 counselors.html 細節**~~：✅ 2026-07-16 修復——按鈕標籤已於 A3 統一為「新增諮詢師」；`pendingKeyForEmail()` 抽入 auth-utils 供兩端共用；垃圾桶內重複 email 改提示「請先還原」。
- ~~**A15 firebase.json**~~：✅ 2026-07-16 修復（並擴大）——hosting `ignore` 改 `sync_*.py`，並加入 `PRD/**`、`**/*.md`、`**/*.xlsx`（內部文件與 xlsx 主檔不再被公開部署）。
...
- ~~工作目錄仍在 OneDrive（15.4 中長期風險未解）~~：✅ 2026-07-18 已遷出——現位於 `C:\Projects\Ormap-Career-Assessment-System`，確認在 OneDrive 同步範圍外
