...
// M3 各節口述原文（示範用）
const rawInputs = {
  'm3._raw_1_1': '每天會產出處理完的客訴結案紀錄跟給客戶的回覆。收到的人是客戶，還有後面要接手的同事。處理好之後客戶會留下來，不會流失。每週還會產出客訴趨勢報告，主管會看。如果做錯，客戶會二次投訴，嚴重的會上網公審。',
  'm3._raw_1_2': '接到客訴以後，先查訂單系統看發生什麼事。再看物流狀態跟客戶過去的往來紀錄。判斷是不是要退讓，500 元以內我自己決定。如果要退換貨，就跟物流、倉儲協調，這段要跟他們一起做。處理完在系統寫結案註記。每週還要帶新人做案例回顧。最花時間的是跟物流協調那段。',
...
export function buildDemoCase() {
  const fields = {
    ...
    // 第一章
    'm3.concrete_outputs': O(['處理完的客訴結案紀錄與給客戶的回覆', '每週客訴趨勢報告'], 'M3-01'),
    'm3.observable_changes': O(['客戶願意留下來，不再流失', '後面接手的同事看得懂處理經過'], 'M3-01'),
    'm3.error_consequences': O(['客戶二次投訴', '嚴重時會上網公審，影響商譽'], 'M3-01'),
    'm3.work_steps': O(['1. 接到客訴後先查訂單系統看發生什麼事', '2. 看物流狀態與客戶過去往來紀錄', '3. 判斷是否退讓，500 元以內自行決定', '4. 需退換貨則與物流、倉儲協調', '5. 處理完在系統寫結案註記'], 'M3-02'),
    'm3.collaboration_items': O(['與物流、倉儲協調退換貨'], 'M3-02'),
    'm3.data_objects': O(['訂單系統', '客服工單系統', '客戶往來紀錄'], 'M3-02'),
...
export function buildPathACase() {
  const fields = {
    ...
    // ── M1 問題釐清定位 ──
    // 開場：這個組織在做什麼（示範高品質口述——講給不認識的人聽、具體、不美化）
    'm1._raw_ch0': cf('我們是青田文化，做的是企業品牌的內容，像官網文章、電子報、社群貼文這些，把客戶專業的東西講成一般人看得懂的內容。客戶大部分是中小企業跟專業服務業，他們自己沒有內容團隊，也不知道怎麼把自己的專業講清楚，這是他們最頭痛的地方。我們的做法是既懂內容、又願意花時間搞懂客戶的行業，不是套模板交差，所以做出來的東西比較到位，客戶會一直續約。我們最在乎的是說真話、不誇大，寧可少寫一點，也不要為了漂亮講出做不到的承諾。'),
    'm1.org_what': O('青田文化是企業品牌內容製作公司，把客戶專業的內容改寫成一般人看得懂、能傳達品牌的文章、電子報與社群貼文。', 'M1-00'),
    'm1.org_who': O('主要服務沒有內部內容團隊的中小企業與專業服務業；他們懂自己的專業，卻不知道怎麼對外講清楚。', 'M1-00'),
    'm1.org_value': O('既懂內容製作、又願意花時間理解客戶所在行業，不套模板、內容更到位，因此客戶留存與續約高。', 'M1-00'),
    'm1.org_beliefs': O(['說真話、不誇大', '寧可少寫，也不做出兌現不了的承諾'], 'M1-00'),
    ...
    // 第一類：先檢視既有資源（A 工作流程、B 人員配置與管理）
    'm2._raw_a': cf('企劃階段沒有把規格定下來，所以後面一直改。跨階段的交付標準也沒寫清楚，編輯跟設計常常在等對方。'),
    'm2.strategies_a': cf(['釐清跨部門權責與交付標準', '建構文件模板或導入系統化工具與自動化']),
    'm2.strategies_a_notes': cf({
      '釐清跨部門權責與交付標準': { reason: '延誤的根源是企劃、編輯、設計對「需求算不算定案」沒有共識，交付標準也沒寫下來。', action: '定義需求定案標準與變更流程，由內容主管在兩週內拍板。' },
      '建構文件模板或導入系統化工具與自動化': { reason: '已經有流程，但沒有文件把規格固定下來，所以每次都重談。', action: '做一份需求規格模板，企劃提案時填完才能進入編輯階段。' },
    }),
    'm2._raw_b': cf('主管沒有固定回饋，延誤都是事後才知道。現有編輯的能力沒問題，是沒有人負責統籌。'),
    'm2.strategies_b': cf(['建立更明確的管理與績效改善機制']),
    'm2.strategies_b_notes': cf({
      '建立更明確的管理與績效改善機制': { reason: '延誤都是事後才知道，主管沒有固定回饋節奏。', action: '建立每週進度同步與風險預警會議，讓延誤能提早被看見。' },
    }),
    // 第二類：既有資源改善後仍有缺口，才評估增加供給
    'm2._raw_c': cf('需求控管是長期持續的事情，不是一次性專案，外包不太適合。但短期可以先找顧問幫忙設計流程。'),
    'm2.strategies_c': cf(['尋求單次顧問諮詢或專業診斷']),
    'm2.strategies_c_notes': cf({
      '尋求單次顧問諮詢或專業診斷': { reason: '需求控管流程需要專業設計，但只做一次，不值得為此養人。', action: '委託顧問設計流程，約一個月，完成後由內部承接維運。' },
    }),
    'm2._raw_d': cf('統籌與需求把關是每週都在發生的事，而且要長期累積客戶跟製程知識，需要一個內部的人長期承擔。'),
    'm2.strategies_d': cf(['增設管理、專案整合或跨部門協調角色', '增加人力並同步重整流程與既有分工']),
    'm2.strategies_d_notes': cf({
      '增設管理、專案整合或跨部門協調角色': { reason: '缺的不是執行人力，是沒有人負責優先順序、跨階段協調與需求把關。', action: '開一個內容專案企劃職缺，明確賦予需求定案的決策權。' },
      '增加人力並同步重整流程與既有分工': { reason: '單純加人不會解決需求變更問題，流程不改只是多一個人一起亂。', action: '新人到職前先完成 A 類的流程與模板，讓他有東西可以承接。' },
    }),
    // 第四部分：合作形式與資源配置決策
    // 整合決策口述（示範高品質口述——用講的、帶出人數與預算的直覺）
    'm2._raw_mix': cf('我算過，這件事需要一個內部的人長期扛，就是這個內容專案企劃，一個人全職，主要負責統籌跟需求把關。前期的需求控管流程我想另外找一個外部顧問來設計，因為那是一次性的，設計完交給正職接手就好，不用為了那個養一個人。預算大概抓一個正職的年度人事成本，顧問是一次性的，大概占兩成。所以就是一個正職加一個短期顧問，兩個一起做，不是二選一。'),
    'm2.resource_plan': cf([
      { form: '全職正職', count: '1 人', scope: '長期統籌內容專案排程、需求定案與變更把關、跨階段協調', cost: '年度人事成本（約占本次預算八成）' },
      { form: '固定週期外包／顧問', count: '1 人', scope: '前期需求控管流程設計與教學，完成後交由正職維運', cost: '一次性費用（約占本次預算兩成）' },
    ]),
    'm2.engagement_mix': cf('一位全職內容專案企劃（長期統籌與需求把關）＋ 一次性外部顧問（協助設計需求控管流程）。'),
    'm2.engagement_split': cf('顧問負責前期流程設計與教學，約一個月；正職承接後續維運、跨階段協調與長期知識累積。'),
    'm2.budget_plan': cf('顧問一次性費用約占本次預算兩成，其餘為正職年度人事成本。'),
    'm2.decision_reason': cf('需求控管流程需要專業設計但只做一次，適合外部；統籌與協調是每週持續發生且需累積內部知識，適合正職。兩者互補而非二選一。'),
    'm2.prerequisite_improvements': cf('先定義需求定案標準與變更處理流程，再讓新人進來承接。'),
    'm2.staffing_decision': cf(['採用混合形式（正職與外部資源並行）。', '需要人力與其他改善策略同步進行。']),
    ...
    // ── M3 職務與人才規格 ──
    // 各節口述（示範高品質口述——第一人稱、具體、講得出情境與判斷）
    'm3._raw_1_1': cf('這個角色最後要產出的是每一季內容專案的排程跟交付，還有需求定案後的規格文件。收到的人是編輯、設計，還有客戶。做好之後內容能如期上架，客戶不再抱怨，編輯跟設計也不用一直重做。如果做不好，交付會延誤、客戶續約受影響，團隊也會一直加班、士氣變差。這個角色存在就是要把需求控管好、把跨階段協調起來，讓內容穩定交付；價值是準時率上去、返工跟加班下來，還有把需求控管的做法沉澱成可以複製的機制。'),
    'm3._raw_1_2': cf('接到企劃的需求以後，先確認規格能不能定案，不能定案的先擋下來。然後排進跨階段的排程，把哪個環節卡哪個標出來。再去跟編輯、設計協調各自的投入時程。過程中要一直追進度，看到風險提前預警，不要等延誤了才講。交付完還要做回顧，把這次的問題整理起來修流程。中間跟企劃確認需求、跟編輯設計喬時間這兩段，是要跟他們一起做的。用到的是專案管理排程工具、內容協作平台，還有需求規格文件。'),
    'm3._raw_1_3': cf('對上是內容主管，重大延誤跟合約範圍的問題要回報他。需求能不能定案這件事我要有決定權，超出合約範圍的變更要上報。對下沒有直接帶人，但會協助編輯排解卡點。跟企劃、編輯、設計是平行協調，排程跟優先順序我可以決定。對外偶爾要跟業務一起面對客戶，確認需求跟時程。'),
    'm3._raw_2_1': cf('到職前要對內容製程跟品質有基本認識，還要會專案協調跟排程，不然接不了。公司的內容類型跟客戶脈絡可以到職後再學。內部的製程默契、各角色的習慣，還有客戶過去的需求跟雷點，這些是內部才有的，要交接。有做過其他跨部門專案協調、或有需求管理概念的，可以比較快上手。'),
    'm3._raw_2_2': cf('核心是判斷需求到底能不能定案，還有判斷延誤的風險、要不要往上升。做決定前一定先看排程、看相依關係、看對客戶承諾了什麼。涉及合約範圍或者重大延誤的，一定要停下來上報，不能自己吞。'),
    'm3._raw_2_3': cf('新手常常分不出哪個需求是真的定案、哪個還會改，容易照單全收。成熟一點的可以自己判斷需求能不能定案、看得出延誤風險。更高階的能在需求反覆的情況下還守住定案標準，甚至回頭去修這個標準。'),
    'm3._raw_3_1': cf('專案管理排程工具跟內容協作平台是一定要會的，到職前就要會。用過其他專案管理工具的可以替代。公司內部平台的設定到職後再學就好。'),
    'm3._raw_3_2': cf('要會需求定案跟變更控管的方法，還有跨階段排程跟相依管理。排程要會用里程碑跟緩衝去安排。客戶臨時要改的時候，要先評估對交付的影響，再決定接不接，不能客戶說改就改。'),
    'm3._raw_3_3': cf('新手排程要人幫忙看，容易漏掉相依。成熟的人可以自己維護跨專案排程，相依關係清楚。高階的人會回頭優化排程的方法，讓大家都好用。'),
    'm3._raw_4_1': cf('品質上要同時守住時程跟內容品質，不能為了趕就亂交。延誤發生的時候要主動說明原因、影響跟怎麼補救，不能藏。時程跟範圍衝突的時候，優先守住對客戶的承諾跟品質底線。'),
    'm3._raw_4_2': cf('不能對客戶或團隊做明知做不到的時程承諾。規則沒寫到的灰色地帶，原則是不傷害交付、不誤導客戶。如果被要求接一個明顯做不到的時程，不能私下答應，要回報。'),
    'm3._raw_4_3': cf('不成熟的人被客戶或主管壓一下就亂答應時程。成熟的人會據實說明、守住底線。高階的人在壓力下還守得住，還會去建立機制讓大家不用靠硬扛。'),
    'm3._raw_5_1': cf('需求跟客戶會一直變，不會有人把每一步都講清楚，這個角色要自己看出哪裡要調整、主動去補流程，不能等人交代。'),
    'm3._raw_5_2': cf('延誤常常不是單一原因，是企劃、編輯、設計跟客戶變更綁在一起，要能拆開來找到真正卡住的地方。'),
    'm3._raw_5_3': cf('這個角色沒有正式帶人，卻要讓編輯、設計跟企劃照排程配合，得靠協調跟信任去推動，不是靠職權。'),
    'm3._raw_5_4': cf('客戶臨時改、時程被壓的情況很常見，要在壓力下還守得住判斷，不會一被逼就亂答應。'),
    'm3._raw_5_5': cf('每個決定都牽動客戶續約跟團隊成本，要能權衡要不要接變更、值不值得趕，而不是照單全收或一律拒絕。'),
    'm3.concrete_outputs': O(['每季內容專案的排程與交付', '定案後的需求規格文件'], 'M3-01'),
    'm3.observable_changes': O(['內容能如期上架，客戶不再抱怨', '編輯與設計不用反覆重做'], 'M3-01'),
    'm3.error_consequences': O(['交付延誤，客戶續約受影響', '團隊加班與士氣受損'], 'M3-01'),
    'm3.work_steps': O(['1. 接到企劃需求後確認規格是否可定案', '2. 排入跨階段排程並標出相依關係', '3. 協調編輯與設計的投入時程', '4. 追蹤進度、發現風險提前預警', '5. 交付後彙整回顧並修正流程'], 'M3-02'),
    'm3.collaboration_items': O(['與企劃確認需求定案', '與編輯、設計協調時程'], 'M3-02'),
    'm3.data_objects': O(['專案管理與排程工具', '內容協作平台', '需求規格文件'], 'M3-02'),
