...
// ═══════════ ① 引導與思考 ═══════════
// M3「結果與價值」§1.1 未填時，印引導而非乾巴巴的「待確認」，讓使用者知道要回哪裡補
function valueOutputsBlock(c) {
  const a = (fieldVal(c, 'm3.value_outputs') || []).filter(x => (x ?? '').toString().trim());
  return a.length ? listBlock(a) : `<p class="tbc">待確認（在「結果與價值」§1.1 回答後，這裡會自動帶入）</p>`;
}
...
export function buildGuideDoc(c) {
  ...
  <h3>預期價值產出</h3>${valueOutputsBlock(c)}
  <h3>工作流程</h3>
  <table>
 ...
 *   使用者的手動文字編輯完整保留，只是替結構標籤補上 inline 樣式。
 *
